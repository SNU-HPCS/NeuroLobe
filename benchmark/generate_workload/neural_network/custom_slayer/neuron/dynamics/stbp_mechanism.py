...
class Accelerated:
    ...
    # Accelerated.stbp_mechanism.dynamics(...) -> handles autograd
    # Accelerated.stbp_mechanism.fwd(...)
    # Accelerated.stbp_mechanism.bwd(...)
    module = None
...
def dynamics(input, decay, state, w_scale, threshold=None, debug=False):
    ...
    _STBPMechanism.DEBUG = debug

    if torch.numel(state) == 1:
        state = state * torch.ones(input.shape[:-1]).to(input.device)

    if input.is_cuda is False or debug is True:
        output = _STBPMechanism.apply(input, decay, state, threshold, w_scale)
    else:
        output = Accelerated.stbp_mechanism.dynamics(
...
class _STBPMechanism(torch.autograd.Function):
    """ """
    DEBUG = False
    ...
    def forward(ctx, input, decay, state, threshold, w_scale):
        """ """
        if quantconfig.QUANTIZE:
            output = _li_dynamics_fwd(input, decay, state, threshold, w_scale, dtype=torch.int64)
        else:
            output = _li_dynamics_fwd(input, decay, state, threshold, w_scale, dtype=torch.float)

        if _STBPMechanism.DEBUG is True and input.is_cuda is True:
            _output, *_ = Accelerated.stbp_mechanism.fwd(
    ...
    def backward(ctx, grad_output):
        ...
        if _STBPMechanism.DEBUG is True and grad_output.is_cuda is True:
            _grad_input, _grad_decay = Accelerated.stbp_mechanism.bwd(
...
def _li_dynamics_fwd(
    ...
    if quantconfig.QUANTIZE:
        ...
        decay_int = 1 - decay.clone().detach().to(dtype).to(input.device)
    ...
    for n in range(input.shape[-1]):
        if quantconfig.QUANTIZE:
            ...
            output_new = output_old * decay_int + \
                (w_scale * input[..., n]).to(dtype)
        if threshold > 0:
...
def _li_dynamics_bwd(grad_output, output, decay):
    ...
    grad_decay = troch.zeros_like(grad_output)
