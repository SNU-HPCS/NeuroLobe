...
__global__ void LIDynamicsFwdKernel(
    ...
    float output_new, output_sign, decay_int, decayed_output;
    int linear_id;

    if(num_decays > 1) {  
        // individual decays or channelwise decay
        // num_decays * decay_block == num_neurons
        decay_int = 1 - decay[(neuron_id) % neurons_per_batch / decay_block];
    } else { // shared decays
        decay_int = 1 - decay[0];
    }

    //if(neuron_id == 0) {
    //    printf("%f, %f, %f, %f\n", decay_int, decay[0], threshold, w_scale);
    //}
    ...
    for(int n=0; n<num_steps; ++n) {
        ...
        decayed_output = output_sign * output_old * decay_int;

        output_new = output_sign * decayed_output + w_scale * input[linear_id];
...
__global__ void LIDynamicsBwdKernel(
    ...
    for(int n=num_steps-1; n>=0; --n) {
        linear_id = n + neuron_id * num_steps;
        grad_input_tensor[linear_id] = grad_output[linear_id];
    }
}
