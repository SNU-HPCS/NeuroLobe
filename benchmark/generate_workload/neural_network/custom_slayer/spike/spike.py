...
def _spike_backward(
    ...
    if quantconfig.loss_config.LOSS == "EXP":
        if graded_spike is False:
            return scale_rho/2/tau_rho * torch.exp(- torch.abs(voltage - threshold) / tau_rho)
        return scale_rho * torch.exp(-torch.clamp(voltage - threshold, max=0) / tau_rho)

    elif quantconfig.loss_config.LOSS == "THR":
        if graded_spike is False:
            return scale_rho/tau_rho * (torch.abs(voltage - threshold) < tau_rho/2)
        return scale_rho/tau_rho * (torch.abs(voltage - threshold) < tau_rho/2)
