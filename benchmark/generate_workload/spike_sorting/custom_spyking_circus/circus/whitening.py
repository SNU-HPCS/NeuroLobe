...
# The waveform procedure is not used in fitting
# We simply use the spatial and temporal whitening
def main(params, nb_cpu, nb_gpu, use_gpu):
    ...
    # the list of channels (this may include the TTL channels)
    ...
    realtime = params.getboolean('whitening', 'realtime')
    ...
    # list of nearby local channel ids for each channel
    nodes_indices = {}
    for elec in numpy.arange(N_e):
        nodes_indices[elec] = inv_nodes[edges[nodes[elec]]]

    print("do_temporal_whitening:", do_temporal_whitening)
    print("do_spatial_whitening:", do_spatial_whitening)
    ...
    #numpy.random.seed(0)
    numpy.random.seed(comm.rank) # comm rank dependent numpy random seed
    ...
    for gidx in [all_chunks[comm.rank]]:
        ...
        # retrieve the timing of all the peaks (for all the electrodes)
        ...
        if len(local_peaktimes) > 0:
            ...
            all_times = numpy.zeros((N_e, len(local_chunk)), dtype=numpy.bool_)
    ...
    if comm.rank == 0:
        ...
        # write the spatial and temporal whitening matrix
    ...
    for gcount, gidx in enumerate(to_explore):
        ...
        if (elt_count_pos + elt_count_neg) < nb_elts:
            ...
            for i in range(N_e):
                ...
                # found_peak_amplitudes =>
                # the electrode data between the peaks
            ...
            if len(local_peaktimes) > 0:
                ...
                all_times = numpy.zeros((N_e, diff_times), dtype=numpy.bool_)
                ...
                if sort_waveforms:
                    ...
                    if realtime:
                        shuffling = numpy.arange(n_times)
                    else:
                ...
                for midx, peak in zip(argmax_peak, all_idx):
                    ...
                    if sign_peaks == 'negative':
                        if N_e > 1:
                            ...
                            # retrieve the electrode with the minimum peak
                        else:
                            elec = 0
                        negative_peak = True
                    elif sign_peaks == 'positive':
                    ...
                    if groups[elec] < upper_bounds:
                        ...
                        #
                        if not myslice.any():
                            # sub_mat is the nearby voltages around the peak (for the selected)
                            ...
                            if not is_noise:
                                ...
                                # the nearby electrodes are set to True
