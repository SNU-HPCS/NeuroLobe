    ...
    if data == 'templates':
        ...
        if os.path.exists(file_name):
            ...
            if local_rank == 0:
                ...
                else: # reconstruct template here? using temp_data, temp_x, temp_y
...
def collect_data(nb_threads, params, erase=False, with_real_amps=False, with_voltages=False, benchmark=False):
    ...
    if debug:
        result_debug = {
            ...
            'success_flags': [numpy.empty(shape=0, dtype=numpy.bool_)],
        }
    else:
        result_debug = None
    ...
    for count, node in enumerate(to_explore):
        ...
        if debug:
            for (key, filename_formatter, dtype) in [
                ...
                ('success_flags', '.success_flags_debug_%d.data', numpy.bool_),
            ]:
...
def get_overlaps(
    ...
    # best_elec => the electrodes w/ valid clusters
    if extension == '-merged':
    ...
    for ielec in to_explore:
        ...
        if len_local > 0:
            ...
            #print("loc_templates shape", templates[:, local_idx].shape)
            #print("all_delays:", all_delays)
