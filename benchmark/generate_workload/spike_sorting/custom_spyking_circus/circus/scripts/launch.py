...
from circus.shared.messages import print_error, print_info, print_and_log, get_colored_header, init_logging, print_debug
...
def main(argv=None):
    ...
    all_steps = [
        ...
        'merging', 'validating', 'thresholding', 'synthetic_temp_gen', 'matching', 'pairwise_correlation'
    ]
 ...
 - (extra) validating [to compare performance with GT neurons]'''
    ...
    if tasks_list is not None:
        ...
        subtasks = [('filtering', 'mpirun'),
                    ...
                    ('validating', 'mpirun'),
                    ('thresholding', 'mpirun'),
                    ('synthetic_temp_gen', 'python'),
                    ('matching', 'python'),
                    ('pairwise_correlation', 'python')]
        ...
        if not result:
            for subtask, command in subtasks:
                if subtask in steps:
                    print_debug()
                    print(subtask, filename, nb_cpu, nb_gpu, use_gpu)
                    if command == 'python':
