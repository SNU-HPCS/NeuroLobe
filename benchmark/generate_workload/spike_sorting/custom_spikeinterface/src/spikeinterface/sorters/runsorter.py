...
def run_sorter_local(
    ...
    **sorter_params,
    ...
    if sorter_params["mode"] == "inference":
        inference_mode = True
    ...
    # set params for spikeinterface here
    # set parameter using sorter params, save it into spikeinterface_params.json file
    if inference_mode:
        if output_folder == None:
            output_folder = "./spykingcircus_output" # hard-coded for spykingcircus
        output_folder = Path(output_folder)

    SorterClass.set_params_to_folder(recording, output_folder, sorter_params, verbose) #
    # if not inference_mode:
    SorterClass.setup_recording(recording, output_folder, verbose=verbose, inference_mode = inference_mode)

    # run spikeinterface, get params from json file?
    SorterClass.run_from_folder(output_folder, raise_error, verbose)
    # output saved in 'sorting'
    if with_output:
