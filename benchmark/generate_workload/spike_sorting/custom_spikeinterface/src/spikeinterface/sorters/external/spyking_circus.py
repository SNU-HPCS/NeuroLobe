...
class SpykingcircusSorter(BaseSorter):
    ...
    _default_params = {
        ...
        # required parameters for spykingcircus
        "sparsify" : 0.10,
        "realtime": False, # whether it will operate with realtime algorithm or offline algorithm(original)
        "spatial": False, # whether it will operate with sparse algorithm or dense algorithm(original)
        "valid_multiple": False, # within min_sps and max_sps -> template match
        "duration": 1000,
        "mode": "full", 
    ...
    @classmethod
    def _setup_recording(cls, recording, sorter_output_folder, params, verbose, inference_mode):
        p = params
        ...
        if not inference_mode:
            ...
            nb_chunks = n_frames // chunk_size # iterate 'nb_chunks' times

            with NpyAppendArray(npy_file) as npaa:
                pbar = tqdm(range(nb_chunks + 1))
                for i, pbar_i in enumerate(pbar):
            ...
            # print("saving recordings.npy done")
    ...
    def _run_from_folder(cls, sorter_output_folder, params, verbose):
        ...
        # make a shell script(run_spykingcircus.sh) and run it with specified number of workers
        ...
        if "win" in sys.platform and sys.platform != "darwin":
            ...
            if params["mode"] == "full": # full -> without merging
                shell_cmd = """
                        #!/bin/bash
                        spyking-circus {recording} -c {num_workers}
                    """.format(
            ...
            elif params["mode"] == "training": # training mode
                shell_cmd = """
                        #!/bin/bash
                        spyking-circus {recording} -c {num_workers} -m filtering,whitening,clustering
                    """.format(
                    recording=sorter_output_folder / "recording.npy", num_workers=num_workers
                    )
            elif params["mode"] == "inference": # inference mode
                shell_cmd = """
                        #!/bin/bash
                        spyking-circus {recording} -c {num_workers} -m fitting
                    """.format(
                    recording=sorter_output_folder / "recording.npy", num_workers=num_workers
                    )
            else:
