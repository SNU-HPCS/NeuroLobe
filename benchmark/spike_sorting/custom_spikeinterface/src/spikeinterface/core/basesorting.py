...
class BaseSorting(BaseExtractor):
    ...
    # for debugging
    def custom_get_all_spike_trains(self, outputs="unit_id"):
        spike_times = [[] for _ in range(len(self.unit_ids))]
        for segment_index in range(self.get_num_segments()):
            for i, unit_id in enumerate(self.unit_ids):
                st = self.get_unit_spike_train(unit_id=unit_id, segment_index=segment_index)
                spike_times[i].append(st)

        for i in range(len(self.unit_ids)):
            spike_times[i] = np.concatenate(spike_times[i])
