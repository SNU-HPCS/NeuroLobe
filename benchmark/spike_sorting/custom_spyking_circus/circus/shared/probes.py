...
def get_nodes_and_edges(parser, validating=False, shank_with=None):
    ...
    """
    # not that the nodes may not include all the channels,
    # rather some TTL channels or channels in other shank (if specified) may be excluded
    ...
    if shank_with is None:
        for key in list(parser.probe['channel_groups'].keys()):
            ...
            for i in parser.probe['channel_groups'][key]['channels']:
                # get edges for the given channel id
