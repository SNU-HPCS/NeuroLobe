...
def create_connections(temp_shape: list, templates: list):
    ...
    num_bci = n_e
    num_types = 3
    num_unit = [num_elec, num_temp, num_bci]
    ...
    for t1 in range(num_types):
        for t2 in range(num_types):
            t1_to_t2_conn_list = []
            if (t1 == 0 and t2 == 1): # elec to temp conn
            ...
            if (t1 == 1 and t2 == 0): # temp to elec conn
                for i in temp_to_elec.keys():
                    for j in list(temp_to_elec[i]):
                        #load = {'template' : templates_memory[i][j]}
                        load = [('template', list(templates_memory[i][j]), 10)]
                        t1_to_t2_conn_list.append((i, j, True, load))
                conn_list[t1][t2] = t1_to_t2_conn_list
            if (t1 == 2 and t2 == 0): # elec to temp conn
