...
def pairwise_correlation(spike_mat, cell_num, duration, window, corr_period, conn_list):
    ...
    #     for target in range(cell_num):
    #         if target != reference and conn_list[reference][target]:
    #             for timestep in range(spike_total):
    #                 if spike_mat[reference][timestep] > 0:
    #                     # if reference neuron spiked
    #                     target_spike = []
    #                     for t in range(timestep - window, timestep + window + 1):
    #                         if t < 0 or t >= spike_total:
    #                             target_spike.append(0)
    #                         else:
    #                             target_spike.append(spike_mat[target][t])
    #                     for w in range(len(target_spike)):
    #                         debug_corr[reference][target][w] += int(target_spike[w] * spike_mat[reference][timestep])
    ...
    # print("\nDifference")
    # diff = np.nan_to_num(corr, nan=0, posinf=0, neginf=0) - np.nan_to_num(debug_corr, nan=0, posinf=0, neginf=0)
    # print(np.argwhere(abs(diff) > 0.000000001))
    # # print(diff)
