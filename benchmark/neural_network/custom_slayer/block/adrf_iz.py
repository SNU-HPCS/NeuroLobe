...
def _doc_from_base(base_doc):
    ...
    return base_doc.__doc__.replace(
        'Abstract', 'Adaptive Resonate & Fire Izhikevich'
    ).replace(
        'neuron parameter', 'Adaptive RF-Izhikevich neuron parameter'
    ).replace(
...
class Input(AbstractADRFIZ, base.AbstractInput):
    def __init__(self, *args, **kwargs):
...
class Dense(AbstractADRFIZ, base.AbstractDense):
    def __init__(self, *args, **kwargs):
...
class Conv(AbstractADRFIZ, base.AbstractConv):
    def __init__(self, *args, **kwargs):
...
class ConvT(AbstractADRFIZ, base.AbstractConvT):
    def __init__(self, *args, **kwargs):
        super(ConvT, self).__init__(*args, **kwargs)
        self.synapse = synapse.ConvT(**self.synapse_params)
        if 'pre_hook_fx' not in kwargs.keys():
            self.synapse.pre_hook_fx = self.neuron.quantize_8bit
        del self.synapse_params
...
class Pool(AbstractADRFIZ, base.AbstractPool):
    def __init__(self, *args, **kwargs):
...
class Unpool(AbstractADRFIZ, base.AbstractUnpool):
    def __init__(self, *args, **kwargs):
...
class KWTA(AbstractADRFIZ, base.AbstractKWTA):
    def __init__(self, *args, **kwargs):
...
class Recurrent(AbstractADRFIZ, base.AbstractRecurrent):
    def __init__(self, *args, **kwargs):
