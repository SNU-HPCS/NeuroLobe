...
def _ath_dynamics_fwd(
    ...
    if quantconfig.QUANTIZE:
        ...
        th_decay_int = 1 - th_decay.clone().detach().to(dtype).to(input.device)
        ref_decay_int = 1 - ref_decay.clone().detach().to(dtype).to(input.device)
