...
__global__ void AThDynamicsFwdKernel(
    ...
    if(num_decays > 1) {  
        ...
        th_decay_int = 1 - th_decay[(neuron_id) % neurons_per_batch / decay_block];
        ref_decay_int = 1 - ref_decay[(neuron_id) % neurons_per_batch / decay_block];
    } else { // shared decays
        th_decay_int = 1 - th_decay[0];
        ref_decay_int = 1 - ref_decay[0];
    }
    ...
    for(int n=0; n<num_steps; ++n) {
        ...
        ref_new = (ref_new * ref_decay_int);
        th = ((th - th0) * th_decay_int) + th0;
