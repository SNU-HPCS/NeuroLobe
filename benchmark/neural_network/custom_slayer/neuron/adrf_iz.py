...
from . import base
from .dynamics import adaptive_resonator
...
class Neuron(base.Neuron):
    ...
    def __init__(
        ...
        graded_spike=False,
        log_init=True, no_spike = False, loss="EXP",
    ):
    ...
    def cx_sin_decay(self):
        ...
        if quantconfig.QUANTIZE:
            ...
            val = quantize(self.sin_decay).cpu().data.numpy().astype(float)
        if len(val) == 1:
            return val[0]
        return val
    ...
    def cx_cos_decay(self):
        ...
        if quantconfig.QUANTIZE:
            ...
            val = quantize(self.cos_decay).cpu().data.numpy().astype(float)
        if len(val) == 1:
            return val[0]
        return val
    ...
    def cx_threshold_decay(self):
        ...
        if quantconfig.QUANTIZE:
            ...
            val = quantize(self.threshold_decay).cpu().data.numpy().astype(float)
        if len(val) == 1:
            return val[0]
        return val
    ...
    def cx_refractory_decay(self):
        ...
        if quantconfig.QUANTIZE:
            ...
            val = quantize(self.refractory_decay).cpu().data.numpy().astype(float)
        if len(val) == 1:
            return val[0]
        return val
    ...
    def v_th_step(self):
        ...
        if quantconfig.QUANTIZE:
            ...
            return self.threshold_step * self.w_scale
    ...
    def dynamics(self, input):
        ...
        real, imag, threshold, refractory = adaptive_resonator.dynamics(
            ...
            self.refractory_state, self.threshold_state,
            self.threshold_step,
            self.threshold,
            self.s_scale,
            debug=self.debug
    ...
    def spike(self, real, imag, threshold, refractory):
        ...
        if self.persistent_state is True:
            with torch.no_grad():
                ...
                # self.imag_state = (
                #         spike[..., -1] * (threshold-1)
                #         + (1 - spike[..., -1]) * self.imag_state
                #     ).detach().clone()
                self.refractory_state = adaptive_resonator.\
                    persistent_ref_state(
                        self.refractory_state, spike[..., -1],
                        self.threshold_state
                    ).detach().clone()
                self.threshold_state = adaptive_resonator.\
                    persistent_th_state(
